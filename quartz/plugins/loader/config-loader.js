...
export async function loadQuartzLayout(layoutOverrides) {
    ...
    for (const pageType of Object.keys(byPageType)) {
        ...
        if (!pt.head)
            pt.head = head;
        if (!pt.header)
            pt.header = defaultLayout.header;
        if (!pt.footer)
            pt.footer = defaultLayout.footer;
    }
...
export function buildLayoutForEntries(entries, layoutConfig) {
    const positions = {
        header: [],
        left: [],
        right: [],
...
/** @internal Exported for testing only. */
export function resolveGroups(items, groups) {
