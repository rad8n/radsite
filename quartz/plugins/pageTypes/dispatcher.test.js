...
const StubB = (() => null);
const StubHead = (() => null);
function makePageType(overrides = {}) {
    return {
        name: "test-page-type",
        layout: "content",
        match: () => true,
...
describe("resolveLayout", () => {
    ...
    test("byPageType override replaces footer", () => {
        const result = resolveLayout(makePageType(), { head: StubHead, footer: [StubA] }, { content: { footer: [StubB] } });
        assert.deepStrictEqual(result.footer, [StubB]);
    });
    test("byPageType override clears footer with []", () => {
        const result = resolveLayout(makePageType(), { head: StubHead, footer: [StubA] }, { content: { footer: [] } });
        assert.deepStrictEqual(result.footer, []);
    });
    test("byPageType override clears header with []", () => {
        const result = resolveLayout(makePageType(), { head: StubHead, header: [StubA] }, { content: { header: [] } });
        assert.deepStrictEqual(result.header, []);
    });
...
describe("resolveLayout frame resolution", () => {
    test("config override frame wins over page type frame", () => {
        const result = resolveLayout(makePageType({ frame: "minimal" }), { head: StubHead }, { content: { frame: "full-width" } });
